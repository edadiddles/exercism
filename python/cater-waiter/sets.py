...
def singleton_ingredients(dishes, intersection):
    ...
    sym_diff_set = set()
    union_intersect_set = set()
    for idx in range(1, len(dishes)):
        if not sym_diff_set:
            sym_diff_set |= set(dishes[idx-1])

        sym_diff_set ^= set(dishes[idx])
        union_intersect_set |= dishes[idx-1] & dishes[idx]


    return sym_diff_set - union_intersect_set - intersection
